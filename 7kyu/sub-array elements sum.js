...
function elementsSum(arr, d = 0){
  //your code here
	...
	console.log("longueur =>", arrLength);
	let sum = 0;
  arr.forEach(el => {
		if (isNaN(el[arrLength - 1])) {
			d != 0 ? sum += d : sum += 0;
		} else {
			sum += el[arrLength - 1];
		}
		console.log("sum boucle =>", sum);
		arrLength --;
	});
...
assert.equal(elementsSum([[3, 2, 1, 0], [4, 6, 5, 3, 2], [9, 8, 7, 4]]), 16);
assert.equal(elementsSum([[3, 2, 1, 7], [4, 6, 5, 3, 2], [9, 8, 7, 4], [2, 5, 12]]), 22);
assert.equal(elementsSum([[3], [4, 6, 5, 3, 2], [9, 8, 7, 4]]), 15);
assert.equal(elementsSum([[3, 2, 1, 0], [4, 6, 5, 3, 2], []]), 7);
assert.equal(elementsSum([[3, 2, 1, 0], [4, 6, 5, 3, 2], []], 5), 12);
assert.equal(elementsSum([[3, 2], [4], []]), 0);
